...
RUN adduser --disabled-password --gecos '' user && mkdir /content && chown -R user:user /content && \
	apt-get update && apt-get install -y --no-install-recommends libgl1 libglib2.0-0 aria2 wget git git-lfs python3-pip python-is-python3
...
RUN git clone -b dev https://github.com/camenduru/SyncDreamer-hf /content/SyncDreamer-hf && \
	...
	pip install -q scikit-image pytorch_lightning transformers taming-transformers-rom1504 kornia webdataset gradio && \
